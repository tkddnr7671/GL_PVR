...
def train(hparams):
    ...
    for epoch in range(max_epoch):
        ...
        for bter in range(nBatch):
            ...
            if modelType == 'LSTM' or modelType == 'RNN':
                output, sr = model(batch_data)
                output = capacity*output.squeeze()

                sr     = sr.squeeze()
                diff   = torch.log(batch_auxil+1) - torch.log(sr+1)
                srloss = (diff**2).sum()

                sp_loss += criterion(output, batch_label)
                loss += criterion(output, batch_label) + gamma*srloss

            elif modelType == 'gmt_rnn' or modelType == 'gmt_lstm' or modelType == 'gmt_gru' or modelType == 'gmt_lr' or modelType == 'gmt_rl':
                output, sr = model(batch_data)
                output = capacity*output.squeeze()
                ...
                #diff   = torch.log(batch_auxil+1) - torch.log(sr+1)
                #srloss = (diff**2).sum()
                srloss = criterion(sr, batch_auxil)

                sp_loss += criterion(output, batch_label)
                loss += criterion(output, batch_label) + gamma*srloss

            elif modelType == 'hybrid_lstm' or modelType == 'hybrid_rnn':
                sr = model(batch_data)
                temp = batch_data.squeeze()*sV[None,None,:] + mV[None,None,:]
                output = capacity*sr.squeeze()*(1+0.00415*(temp[:,:,1]-25))/1000
        ...
        val_loss = 0
        val_sp_loss = 0
        nBatch = np.floor((nData-BatchSize)/6).astype(int)
        for bter in range(nBatch):
            stridx = bter*6
            endidx = bter*6 + BatchSize
            ...
            if modelType == 'LSTM' or modelType == 'RNN':
                ...
                val_sp_loss += criterion(output, batch_label)
                val_loss += criterion(output, batch_label) + gamma*srloss
                ...
                sr = model(batch_data)
                temp = batch_data.squeeze()*sV[None,None,:] + mV[None,None,:]
                output = val_capacity*sr.squeeze()*(1+0.00415*(temp[:,:,1]-25))/1000
        ...
        val_sp_loss /= (val_i*nBatch*seqLeng)

        if val_sp_loss < prev_loss:
            ...
            torch.save(model_dict, savePath)
            prev_loss = val_sp_loss

        losses.append(loss.item())
        sp_losses.append(sp_loss.item())
        val_losses.append(val_loss.item())
        val_sp_losses.append(val_sp_loss.item())
        print(f'Epoch [{epoch+1}/{max_epoch}], MSE: {loss.item():.4f}, {sp_loss.item():.4f}, {val_loss.item():.4f}, {val_sp_loss.item():.4f}')

    if hparams['save_losses']:
        trn_loss = np.array(losses)
        trn_sp_loss = np.array(sp_losses)
        val_loss = np.array(val_losses)
        val_sp_loss = np.array(val_sp_losses)
        savepath = os.path.join(hparams['save_dir'], 'trn_loss.npy')
        np.save(savepath, trn_loss)
        savepath = os.path.join(hparams['save_dir'], 'trn_sp_loss.npy')
        np.save(savepath, trn_sp_loss)
        savepath = os.path.join(hparams['save_dir'], 'val_loss.npy')
        np.save(savepath, val_loss)
        savepath = os.path.join(hparams['save_dir'], 'val_sp_loss.npy')
        np.save(savepath, val_sp_loss)
...
def test(hparams):
    ...
    result = []
    labs   = []
    nBatch = np.floor((nData-BatchSize)/24).astype(int)
    for bter in range(nBatch):
        stridx = bter*24
        endidx = bter*24 + BatchSize
        ...
        if modelType == 'LSTM' or modelType == 'RNN':
            ...
            sr = model(batch_data)
            temp = batch_data.squeeze()*sV[None,None,:] + mV[None,None,:]
            output = tst_capacity*sr.squeeze()*(1+0.00415*(temp[:,:,1]-25))/1000
